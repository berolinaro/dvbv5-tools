...
namespace Util {
	...
	void hexdump(void const * const data, size_t size, std::ostream &where, std::string const &indent, uint8_t bytesPerRow) {
		...
		SaveIOState isr(where);
		unsigned char tmp[17];
		uint8_t const maxOffset = bytesPerRow-1;
		tmp[16]=0;
		...
		for(size_t i=0; i<size; i++) {
			...
			if(isprint(c))
				...
				tmp[i%bytesPerRow]='.';
			where << std::setw(2) << static_cast<uint16_t>(c) << " ";
			if(i%bytesPerRow == maxOffset) {
				where << tmp << std::endl;
				if(i<size-1)
					where << indent;
			}
		...
		if(size%bytesPerRow) {
			tmp[size%bytesPerRow]=0;
			for(int i=0; i<bytesPerRow-(size%bytesPerRow); i++)
				where << "   ";
			where << tmp << std::endl;
		}
	}

	uint32_t crc32(unsigned char const * const data, size_t size, uint32_t previous) {
